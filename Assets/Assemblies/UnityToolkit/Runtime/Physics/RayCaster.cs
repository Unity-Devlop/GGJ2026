...
namespace UnityToolkit
{
    public static class RayCaster2D
    {
            ...
            LayerMask layerMask)
        {
            ...
            int count = Physics2D.OverlapCircle(position, radius, contactFilter, collider2Ds);
            cols = collider2Ds;
            return count;
        }
            ...
            LayerMask layerMask)
        {
            ContactFilter2D contactFilter = new ContactFilter2D();
            contactFilter.SetLayerMask(layerMask);
            int ans = Physics2D.OverlapBox(point, size, facingAngle, contactFilter, collider2Ds);
            results = collider2Ds;
            return ans;
        }
    }

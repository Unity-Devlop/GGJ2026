...
namespace GGJ2026.GamePlay
{
    ...
    public class 无常自力 : ICardEffectExecutor
    {
        public async UniTask<bool> Execute(CardData cardData, IEntityController atk, IEntityController tar)
        {
            ...
            if (atk.TryGetLastUsedCardThisRound(out var lastCard))
            {
                await CardEffects.ExecuteCardEffects(new CardData(lastCard), atk, tar);
            }

            if (Random.Range(0, 100) < cardData.config.Value[0])
            {
                return true;
            }

...
namespace UnityToolkit
{
    public class AddressablesUILoader : IUILoader
    {
        ...
        public GameObject Load<T>() where T : IUIPanel
        {
            ...
            var prefab = Addressables.LoadAssetAsync<GameObject>($"UI/{name}/{name}.prefab").WaitForCompletion();
        ...
        {
            string name = typeof(T).Name;
            Addressables.LoadAssetAsync<GameObject>($"UI/{name}/{name}.prefab").Completed += handle =>
            {
        ...
        {
            string name = typeof(T).Name;
            var task = Addressables.LoadAssetAsync<GameObject>($"UI/{name}/{name}.prefab").Task;
